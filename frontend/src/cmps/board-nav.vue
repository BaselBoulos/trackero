<template>
	<div class="board-nav clearfix">
		...
		>
			<span class="icon-star board-header-btn-icon" :class="{ 'icon-starred': isBoardStarred }">
				<svg
					v-if="isBoardStarred"
					version="1.1"
					id="Capa_1"
		...
		<div class="board-header-btns mod-left">
         ...
         > -->
			<template v-for="(member, idx) in memberList">
				<a class="member member-draggable" :style="{ 'z-index': memberList.length - idx }" :key="member._id">
			...
			<div v-if="!breakpointBig && !showListMembers">
				<a @click="toggleListMembers" class="board-header-btn members-list-svg">
					<svg stroke="white" fill="white" stroke-width="0" viewBox="0 0 512 512" height="16px" width="16px" xmlns="http://www.w3.org/2000/svg">
						<path
			...
			<a v-if="breakpointBig" class="board-header-btn-invite manage-board-members" title="Invite to board">
			...
			<span class="board-header-special-btn-container">
				<div class="board-header-filter-btn">
					<button type="button" class="board-header-btn header-filter-btn board-header-special-btn">
						...
						<span class="board-header-btn-text-filter" v-if="breakpointBig">Filter</span>
					</button>
				</div>
			</span>
			<a @click="toggleBoardNavMenu" class="board-header-btn mod-show-menu show-sidebar">
				<span class="icon-overflow-menu icon-sm board-header-btn-icon"></span>
