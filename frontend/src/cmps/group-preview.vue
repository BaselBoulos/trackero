<template>
	<section class="group-preview-container">
		<section class="group-header-section">
			<h2 @mousedown="closeOptions" v-if="isEditable === false" @mouseup="toggleEditing" class="group-header-title-assist">
				{{ group.title }}
			</h2>
			<textarea
				v-else
		...
		>
			<Draggable v-for="task in group.tasks" :key="task.id">
				<task-preview @openPreviewEdit="openPreviewEdit" :task="task" :board="board" :group="group" />
			</Draggable>
			...
			<div class="task-composer-container">
				<div v-if="isTaskInputOpen" class="card-composer-open">
					...
					<div class="add-task-control-section">
						<button @click="addTask" class="add-card-confirm-btn">Add card</button>
